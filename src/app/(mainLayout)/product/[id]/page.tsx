  ...
  const handleReviewSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setReviews([{ rating, comment, user: { firstName: user.firstName }, createdAt: new Date() }, ...reviews]);
      }
    } catch (err: any) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-24">
              ...
              {[product.thumbnail, ...product.images].map((img, i) => (
                <div
                  ...
                  className={`relative w-20 h-20 flex-shrink-0 cursor-pointer border rounded-sm overflow-hidden transition-all ${
                    mainImage === img ? "border-black shadow-md scale-95" : "border-gray-100 opacity-60"
                  }`}
          ...
          <div className="flex flex-col justify-center">
            ...
            <div className="flex items-center gap-3 mb-8">
              ...
              <span className="content-text !text-[12px]">({reviews.length} Verified Reviews)</span>
            </div>

            <div className="flex items-baseline gap-4 mb-10">
              <span className="text-3xl font-bold tracking-tight">${product.salePrice}</span>
            ...
            <div className="space-y-8 mb-12 border-t border-gray-100 pt-8">
              {product.description && (
                <div>
                  <h4 className="text-[11px] uppercase tracking-widest font-black mb-3">The Lowdown:</h4>
                  <p className="content-text leading-relaxed">{product.description}</p>
                </div>
              )}
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <div className="flex items-center justify-between border border-gray-200 rounded-full px-6 py-4 bg-gray-50 min-w-[150px]">
                <button onClick={() => setQuantity(q => q > 1 ? q - 1 : 1)} className="hover:text-gray-400 transition-colors">
                  <FaMinus size={12} />
                </button>
                <span className="font-bold text-lg px-4">{quantity}</span>
                <button onClick={() => setQuantity(q => q + 1)} className="hover:text-gray-400 transition-colors">
                  <FaPlus size={12} />
                </button>
              </div>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  toast.success(`${product.name} added!`, {
                    style: { background: '#000', color: '#fff', borderRadius: '0px', fontSize: '12px' }
                  });
                }}
        ...
        <div className="bg-[#FAF7F2] rounded-3xl p-8 md:p-16">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
             <div className="lg:col-span-4">
                <h3 className="section-title !text-left !text-3xl mb-4">Customer Stories</h3>
                <div className="flex items-center gap-3 mb-10">
                   <div className="flex text-black text-sm">
                      {[...Array(5)].map((_, i) => <FaStar key={i} />)}
                   </div>
                   <span className="font-bold text-sm">{reviews.length} Reviews</span>
                </div>

                {user ? (
                   <form onSubmit={handleReviewSubmit} className="bg-white p-6 rounded-2xl shadow-sm space-y-4">
                      <div className="flex gap-2">
                        {[1,2,3,4,5].map(n => (
                          <FaStar key={n} onClick={() => setRating(n)} className={`cursor-pointer text-xl ${rating >= n ? 'text-black' : 'text-gray-100'}`} />
                        ))}
                      </div>
                      <textarea 
                        value={comment}
                   ...
                   <div className="bg-black text-white p-6 rounded-2xl">
                      <p className="text-sm">Sign in to share your experience with this product.</p>
                      <Link href="/login" className="inline-block mt-4 underline text-xs font-bold uppercase tracking-widest">Login Now</Link>
                   </div>
                )}
             </div>
             ...
             <div className="lg:col-span-8 space-y-8 max-h-[600px] overflow-y-auto pr-4 no-scrollbar">
                {reviews.length > 0 ? reviews.map((rev, i) => (
                   <div key={i} className="bg-white/50 p-6 rounded-2xl border border-white/20">
                         ...
                         <div className="flex items-center gap-3">
                            ...
                            <div>
                               <div className="flex text-black text-[8px] mb-1">
                                  {[...Array(5)].map((_, idx) => <FaStar key={idx} className={idx < rev.rating ? 'text-black' : 'text-gray-200'} />)}
                               </div>
                               <h5 className="font-bold text-[13px]">{rev.user?.firstName || "Verified Buyer"}</h5>
                            </div>
                         </div>
                      ...
                      <p className="content-text !text-[14px] italic leading-relaxed">"{rev.comment}"</p>
                   </div>
                )) : (
                  <div className="h-full flex items-center justify-center border-2 border-dashed border-gray-200 rounded-3xl p-20">
                     <p className="text-gray-400 uppercase tracking-widest text-xs">Be the first to review</p>
                  </div>
                )}
             </div>
          </div>
